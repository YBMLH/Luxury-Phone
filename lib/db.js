...
// (see lib/imageCompress.js) so pages load fast. Destination, in order:
//   1. Cloudinary (free CDN, auto-optimized delivery) if configured
//   2. ImgBB if a key is set
//   3. Firebase Storage as a last resort
// `config` is the site settings object ({ cloudinary, imgbbApiKey }).
export async function uploadProductImage(file, config = {}) {
  ...
  const key = config.imgbbApiKey || process.env.NEXT_PUBLIC_IMGBB_API_KEY;
  if (key) return uploadToImgbb(compressed, key);
