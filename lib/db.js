...
// Gives a readable URL to every product that is still falling back to its
// database id. Products created before slugs existed show up as
// /products/tiKcP0V2p1UolPY543RX, which reads as broken to a customer and
// tells Google nothing about what is on the page.
//
// Existing slugs are never touched — rewriting one would break links already
// shared and anything Google has already indexed.
export async function backfillProductSlugs() {
  const snap = await getDocs(collection(db, 'products'));
  const missing = snap.docs.filter((d) => !d.data().slug);

  let fixed = 0;
  for (const productDoc of missing) {
    const name = productDoc.data().name;
    if (!name) continue;
    // Sequential rather than parallel: generateUniqueSlug checks the
    // collection for collisions, so two products with the same name would
    // otherwise both be handed the identical slug.
    const slug = await generateUniqueSlug(name);
    await updateDoc(doc(db, 'products', productDoc.id), { slug });
    fixed += 1;
  }

  if (fixed) logActivity('product.slug_backfill', { fixed });
  return { fixed, checked: snap.size };
}
...
// Shopfront photos live outside settings/site for the same reason category
// images do. With no image host configured an upload becomes a data: URL,
// and two of those inside the settings document would push it past
// Firestore's 1 MiB single-document limit — which does not just lose the
// photos, it takes the hero text, contact details and opening hours down
// with them, on every page of the site.
export async function getBranchPhotos() {
  const snap = await getDocs(collection(db, 'branchPhotos'));
  const map = {};
  snap.docs.forEach((d) => {
    map[d.id] = d.data().url;
  });
...
export async function setBranchPhoto(index, url) {
  const id = String(index);
  if (!url) {
    await deleteDoc(doc(db, 'branchPhotos', id));
  } else {
    await setDoc(doc(db, 'branchPhotos', id), { url });
  }
  logActivity('settings.branch_photo_update', { branch: id, cleared: !url });
}
