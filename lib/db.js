...
// (see lib/imageCompress.js) so pages load fast. If an ImgBB key is given
// (from the dashboard or the env var), it uploads to ImgBB — free and needs
// no payment card. Otherwise it falls back to Firebase Storage.
export async function uploadProductImage(file, imgbbKey) {
  ...
  const key = imgbbKey || process.env.NEXT_PUBLIC_IMGBB_API_KEY;
  return key ? uploadToImgbb(compressed, key) : uploadToFirebase(compressed);
}
...
async function uploadToImgbb(file, key) {
  ...
  // display_url points at ImgBB's already-resized copy, which loads fastest.
