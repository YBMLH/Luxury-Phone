...
// Picks which products appear in the hero device fan — the fan IS the
// featured section, so it shows exactly what the owner marked as Featured.
// Products without a photo are skipped (there'd be nothing to show). If
// nothing is marked featured yet, it falls back to the newest products so
// the hero is never empty. Shared by the server render
// (app/(store)/page.js) and the live client refresh (HomeClient) so the two
// can never disagree on the set.
export function pickShowcase(products, max = 6) {
  const withImages = (products || []).filter((p) => p.images?.length);
  const featured = withImages.filter((p) => p.featured);
  return (featured.length ? featured : withImages).slice(0, max);
}

// "#RRGGBB" -> "rgba(r, g, b, alpha)", for colored glows/shadows where a
// category/brand accent needs a specific opacity.
